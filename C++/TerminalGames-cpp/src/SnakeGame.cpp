...
void SnakeGame::RunSnakeGame () {
	Window::MenuDisplay snakeMenu ("Snake");
...
void SnakeGame::updateDisplay () {
	...
	if ((this->snakeCoordX[prevHead] == this->snakeCoordX[this->headIndex]) &&
		...
		display->setElem (this->snakeCoordX[prevHead],
						  this->snakeCoordY[prevHead],
						  '-');
	...
	if (this->snakeCoordX[prevHead] + 1 == this->snakeCoordX[this->headIndex]) {
		display->setElem (this->snakeCoordX[this->headIndex],
						  this->snakeCoordY[this->headIndex],
						  '>');
...
void SnakeGame::setTarget () {
	...
	while ((this->targetX != -1) && (this->targetY != -1)) {
		...
		for (int snakeIndex = this->offsetIndex (this->headIndex, -(startLength + this->score - 1));
			 snakeIndex != this->nextIndex (this->headIndex);
			 snakeIndex = this->nextIndex (snakeIndex)) {
			if ((this->snakeCoordX[snakeIndex] == this->targetX) &&
				(this->snakeCoordY[snakeIndex] == this->targetY)) {
				this->targetX = -1;
				this->targetY = -1;
			}
		}
...
bool SnakeGame::moveSnake () {
	...
	if ((newHeadX < 0) || (newHeadX >= display->getWidth ()) ||
		(newHeadY < 0) || (newHeadY >= display->getHeight ())) {
		Sleep (5000);
		return true;
	}
