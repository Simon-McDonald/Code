...
Window::ConsoleWindow::ConsoleWindow (int width, int height) : screenStartY (-1) {
	...
	this->height = (rNew.bottom - rNew.top) / 12 - 4;
...
Window::ConsoleWindow* Window::ConsoleWindow::getWindow () {
	static ConsoleWindow window (500, 400);
	return &window;
} /* Window::ConsoleWindow::getWindow */

void Window::ConsoleWindow::update () {
...
void Window::ConsoleWindow::setElem (int x, int y, char val) {
	this->_setElem (x + 1, y + 1, val);
} /* Window::ConsoleWindow::setElem */

void Window::ConsoleWindow::_setElem (int x, int y, char val) {
	this->_validateCoordinates (x, y);
	this->screenBuffer [(this->height - y - 1) * (this->width + 1) + x] = val;
} /* Window::ConsoleWindow::_setElem */

void Window::ConsoleWindow::setString (int x, int y, const char *str, int len) {
	x += 1, y += 1;
	this->_validateCoordinates (x, y);
...
void Window::ConsoleWindow::addBorder (int x1, int y1, int x2, int y2, int pattern) {
	this->_addBorder (x1 + 1, y1 + 1, x2 + 1, y2 + 1, pattern);
} /* Window::ConsoleWindow::addBorder */

void Window::ConsoleWindow::_addBorder (int x1, int y1, int x2, int y2, int pattern) {
	...
	assert ((sizeof (cornerTR) == sizeof (cornerTL)) &&
			(sizeof (cornerBL) == sizeof (cornerTL)) &&
			(sizeof (cornerBR) == sizeof (cornerTL)) &&
			(sizeof (sideT) == sizeof (cornerTL)) &&
			(sizeof (sideR) == sizeof (cornerTL)) &&
			(sizeof (sideB) == sizeof (cornerTL)) &&
...
void Window::ConsoleWindow::clearScreen () {
...
void Window::ConsoleWindow::setLineElem (int x, int y, char val, int length) {
	this->_setLineElem (x + 1, y + 1, val, length, true);
} /* Window::ConsoleWindow::setLineElem */

void Window::ConsoleWindow::setTitleString (int y, const char *str, int len) {
...
void Window::ConsoleWindow::_setLineElem (int x, int y, char val, int length, bool isHorizontal) {
	this->_validateCoordinates (x, y);
...
void Window::ConsoleWindow::_setString (int x, int y, const char *str, int length, bool isHorizontal) {
	this->_validateCoordinates (x, y);
	this->_validateCoordinates (x + isHorizontal ? 0 : (length - 1), y + isHorizontal ? (length - 1) : 0);
...
inputStruct Window::ConsoleWindow::getUserInput (bool block) {
...
void Window::ConsoleWindow::_validateCoordinates (int x, int y) {
	if (x < 0 || x >= this->width) {
...
int Window::ConsoleWindow::getWidth () {
	return this->width - 2;
} /* Window::ConsoleWindow::getWidth */

int Window::ConsoleWindow::getHeight () {
